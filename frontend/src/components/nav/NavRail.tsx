...
import { getHistory } from '@/hooks/useToast'
...
export function NavRail({ active, onNavigate, downloadsBadge }: NavRailProps): React.ReactElement {
  ...
  // Show badge when there are notifications and user is not on the notifications page
  useEffect(() => {
    const check = (): void => setHasNotifications(getHistory().length > 0 && active !== 'notifications')
    check()
  ...
  return (
    <nav className="w-[54px] bg-surface border-r border-border flex flex-col items-center py-3 gap-1 flex-shrink-0 z-10">
      ...
      <NavItem id="notifications" title="Notifications" active={active === 'notifications'} onClick={() => onNavigate('notifications')} badge={hasNotifications}>
