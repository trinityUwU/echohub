...
interface ModelStats {
  ...
  profiles: Record<string, { best_tps: number; runs: number }>
  latest: number
}
...
export function LeaderboardView({ history }: Props): React.ReactElement {
  ...
  const stats = useMemo((): ModelStats[] => {
    ...
    return Array.from(map.entries()).map(([model_name, runs]) => {
      const tps = runs.map(r => r.tok_per_sec)
      const ttfts = runs.map(r => r.ttft_ms).filter((v): v is number => v != null)
      const profiles: Record<string, { best_tps: number; runs: number }> = {}
      for (const r of runs) {
        const p = r.profile_name ?? 'default'
        if (!profiles[p]) profiles[p] = { best_tps: 0, runs: 0 }
        profiles[p].runs++
        if (r.tok_per_sec > profiles[p].best_tps) profiles[p].best_tps = r.tok_per_sec
      }
      return {
        model_name,
        ...
        best_total_ms: Math.min(...runs.map(r => r.total_ms)),
        profiles,
...
function LeaderboardRow({ stats: m, rank, globalBestTps, expanded, onToggle }: {
  ...
  const medalColor = rank === 1 ? 'text-yellow' : rank === 2 ? 'text-text-secondary' : rank === 3 ? 'text-[#cd7f32]' : 'text-text-muted/40'
  const engineLabel = m.engine === 'vllm' ? 'vLLM' : 'llama.cpp'
    ...
    <div className={`bg-surface border rounded-md overflow-hidden transition-colors ${expanded ? 'border-accent/20' : 'border-border'}`}>
      ...
      <button onClick={onToggle} className="w-full text-left px-4 py-3 hover:bg-elevated/50 transition-colors cursor-pointer">
        <div className="flex items-center gap-3">
          ...
          {/* Rank */}
          <div className={`text-sm font-bold font-mono w-6 flex-shrink-0 ${medalColor}`}>
            {rank <= 3 ? ['🥇', '🥈', '🥉'][rank - 1] : `#${rank}`}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xs text-text-muted/50 w-8 text-right">{barPct}%</span>
            </div>
          </div>
      ...
      {expanded && (
        <div className="border-t border-border/50 px-4 py-3 bg-elevated/30 flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                {profiles.map(([name, p]) => (
                  <div key={name} className="bg-surface border border-border rounded-md px-3 py-2">
                    <div className="text-2xs text-text-muted mb-1">{name}</div>
                    <div className={`text-base font-bold font-mono ${
                      p.best_tps >= 60 ? 'text-green' : p.best_tps >= 30 ? 'text-yellow' : 'text-red'
                    }`}>{p.best_tps}</div>
                    <div className="text-2xs text-text-muted/50">{p.runs} run{p.runs !== 1 ? 's' : ''}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-1">
            ...
            <DetailRow label="Spread" value={`${spread.toFixed(1)} (${spreadPct}%)`} dim={spreadPct > 20} />
...
function DetailRow({ label, value, highlight, dim }: { label: string; value: string; highlight?: boolean; dim?: boolean }): React.ReactElement {
  return (
    <div className="flex items-baseline justify-between py-0.5 gap-2">
      <span className="text-xs text-text-muted">{label}</span>
      <span className={`text-xs font-mono font-medium ${highlight ? 'text-accent' : dim ? 'text-red' : 'text-text-secondary'}`}>{value}</span>
    </div>
  )
}
